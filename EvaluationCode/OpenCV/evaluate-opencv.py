...
# Evaluate the QRCodeDetectorAruco and measure detection and decoding success
def evaluate_qr_detector_aruco(image_dir, annotation_file):
    qr_detector = cv2.QRCodeDetectorAruco()  # Use QRCodeDetectorAruco
    ...
    for image_info in coco_data['images']:
        ...
        # Perform QR code detection and decoding using QRCodeDetectorAruco
        ...
        if points is not None:
            ...
            for i in range(len(decoded_info)):
                detected_points_list.append(points[i].tolist())
                if decoded_info[i]:
        ...
        if decoded_info:
            for data in decoded_info:
                print(f"Detected QR code content: {data}")
...
evaluate_qr_detector_aruco(image_directory, annotation_file)